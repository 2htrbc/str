...
// Main route using clean path: /:stream/index.m3u8
app.get('/:stream/index.m3u8', (req, res) => {
  ...
  request.get(sourceUrl, (err, response, body) => {
    ...
    // Rewrite each media line to proxy segments through our server
...
// Proxy for .ts (or .aac, .vtt, etc.) media segments
...
// Optional: backward compatibility for /playlist.m3u8?stream=ATBS
